...
export class TxObject {
  ...
  constructor(tx: Partial<ITxObject>) {
    ...
    if (tx.gas) this.gas = "0x" + new BN(tx.gas, 10).toString(16);
    if (tx.gasPrice) this.gasPrice = "0x" + new BN(tx.gasPrice, 10).toString(16);
    if (tx.value) this.value = "0x" + new BN(tx.value, 10).toString(16);
